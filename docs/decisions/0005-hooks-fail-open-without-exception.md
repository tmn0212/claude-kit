...
The exit itself is `os._exit(0)` after an explicit `sys.stdout.flush()`, not
`sys.exit(0)`. A deferred flush at interpreter shutdown can hit EPIPE, and
CPython rewrites the status to 120, which the harness reads as a hook failure.
The `prompt_shaper.py` hook found this first and documented it with a
measurement; the six guards claimed to fail open while still using `sys.exit`.

`tests/smoke.py` feeds every hook a garbage payload and asserts exit 0 with
empty stderr, and checks the action name exists in the dispatch table, because
an unknown action also returns 0 and the check would otherwise pass against an
empty table.
